...
## 2026-07-29 - Extension distributed as a packaged zip via GitHub Release
...
## 2026-07-29 - Production backend deployed to self-hosted homelab, `web` on Vercel

`api`/`worker`/`postgres`/`redis` needed a real place to run in production. Full runbooks live in the homelab-config repo (`shekse-code-sync-backend-setup.md`, `shekse-code-sync-deploy-update.md`), not here - this entry covers only the decisions with product/architecture consequences.

| # | Decision | Choice | Why |
|---|---|---|---|
| 88 | Backend runs in Docker Compose on a self-hosted Proxmox LXC, not a managed platform | LXC 155 (`forge`), pulling prebuilt `api`/`worker` images from GHCR (`docker compose pull && up -d` - no build or repo checkout on the box) | Matches the project's self-hosted-first stance (`docs/TECH_STACK.md`); `web` alone moved to Vercel for its own deploy convenience, the stateful/compute-heavier backend stays on hardware already paid for |
| 89 | `scs-api`/`scs-worker` GHCR packages set to public | Changed via GitHub package settings, one-time | The source repo is already public (decision #85), so keeping the images private only adds a PAT-management burden on the homelab box for no confidentiality benefit |
| 90 | `api` exposed publicly via Cloudflare Tunnel, not Tailscale-only | `api-codesync.shekse.com`, reusing the homelab's existing tunnel/Caddy setup | `web` on Vercel is public internet, not on the homelab's private tailnet - Tailscale-only exposure (the pattern used for personal-only homelab services) would have made `api` unreachable from `web` |
| 91 | Production `AI_PROVIDER`/`AI_MODEL` follow the working local dev config, not `.env.example`'s default | `AI_PROVIDER="openai"`, `AI_MODEL="deepseek.v3.2"` against a Bedrock-compatible gateway, same key reused across dev and this backend for now | `.env.example`'s `anthropic`/`claude-opus-4-8` default was never actually provisioned with a working key; matching the already-working local config was simpler than provisioning a new Anthropic key for no immediate benefit. Revisit key separation if cost attribution or blast-radius isolation starts to matter |
| 92 | Production GitHub OAuth App and GitHub App registered separately from dev, pointed at `codesync.shekse.com` | New OAuth App (login) and new GitHub App `sheksecodesync` (App ID 4425162; Contents: Read & write only; webhooks off; installable on any account) | Matches decision #65's requirement that prod never reuse dev credentials; installable-on-any-account chosen to match the PRD's multi-user design even though only one real user exists today |
